...
export default function OrderDetailPage() {
  ...
  const { tokenInfo: inputTokenInfo } = useTokenMetadata(order?.inputToken || "0x0");
  const { tokenInfo: outputTokenInfo } = useTokenMetadata(order?.outputToken || "0x0");

  if (isLoading) {
    return (
      <div className="min-h-screen bg-zinc-50 dark:bg-black">
        <div className="container mx-auto max-w-2xl px-4 py-8">
          <div className="animate-pulse rounded-lg border border-zinc-200 bg-white p-8 dark:border-zinc-800 dark:bg-zinc-900">
      ...
      <div className="min-h-screen bg-zinc-50 dark:bg-black">
        <div className="container mx-auto max-w-2xl px-4 py-8">
          <div className="rounded-lg border border-zinc-200 bg-white p-8 text-center dark:border-zinc-800 dark:bg-zinc-900">
            ...
            <Link className="mt-4 inline-block text-blue-600 hover:underline dark:text-blue-400" href="/">
              Back to orders
            </Link>
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-black">
      <div className="container mx-auto max-w-2xl px-4 py-8">
        <Link
          className="mb-6 inline-flex items-center gap-2 text-zinc-600 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-zinc-50"
          href="/"
        >
          <svg
        ...
        <div className="rounded-lg border border-zinc-200 bg-white p-8 dark:border-zinc-800 dark:bg-zinc-900">
          <div className="mb-6 flex items-center justify-between">
            <h1 className="font-bold text-2xl text-zinc-900 dark:text-zinc-50">Order #{order.orderId.toString()}</h1>
            <div className="flex gap-2">
              {order.fulfilled && (
                <span className="rounded bg-green-100 px-3 py-1 font-medium text-green-800 text-sm dark:bg-green-900 dark:text-green-200">
                  Fulfilled
                </span>
              )}
              {order.cancelled && (
                <span className="rounded bg-red-100 px-3 py-1 font-medium text-red-800 text-sm dark:bg-red-900 dark:text-red-200">
                  Cancelled
                </span>
              )}
              {isActive && (
                <span className="rounded bg-blue-100 px-3 py-1 font-medium text-blue-800 text-sm dark:bg-blue-900 dark:text-blue-200">
                  Active
                </span>
              )}
            </div>
          ...
          <div className="space-y-6">
            <div>
              <h2 className="mb-2 font-medium text-sm text-zinc-500 dark:text-zinc-400">Creator</h2>
              <p className="font-mono text-zinc-900 dark:text-zinc-50">{formatAddress(order.creator)}</p>
            </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="mb-4 font-semibold text-base text-zinc-900 dark:text-zinc-50">
                    Creator wants to <span className="text-green-600 dark:text-green-400">BUY</span>{" "}
                ...
                <div className="border-blue-200 border-t pt-4 dark:border-blue-800">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <h3 className="mb-2 font-medium text-sm text-zinc-500 dark:text-zinc-400">Paying</h3>
                      <p className="font-semibold text-lg text-zinc-900 dark:text-zinc-50">
                        {formatAmount(order.inputAmount, order.inputToken, inputTokenInfo?.decimals)}{" "}
                        {inputTokenInfo?.symbol || "TOKEN"}
                      </p>
                      <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
                        {inputTokenInfo?.name || formatAddress(order.inputToken)}
                      </p>
                    </div>

                    <div className="text-right">
                      <h3 className="mb-2 font-medium text-sm text-zinc-500 dark:text-zinc-400">Receiving</h3>
                      <p className="font-semibold text-lg text-zinc-900 dark:text-zinc-50">
                        {formatAmount(order.outputAmount, order.outputToken, outputTokenInfo?.decimals)}{" "}
                        {outputTokenInfo?.symbol || "TOKEN"}
                      </p>
                      <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
                        {outputTokenInfo?.name || formatAddress(order.outputToken)}
                      </p>
                    </div>
                  </div>
                </div>
            ...
            {isActive && (
              <div className="border-zinc-200 border-t pt-6 dark:border-zinc-800">
